...
class Game
  ...
  def next_step(bukva)
    ...
    if @good_letters.join.include?(bukva) || @bad_letters.join.include?(bukva) || 
       @good_letters.join.include?(bukva_alt) || @bad_letters.join.include?(bukva_alt)
      return
    end
    
    if @letters.join.include?(bukva) || @letters.join.include?(bukva_alt)
      # Если в слове есть буква запишем её в число "правильных" буква
      if bukva_alt == "й" || bukva_alt == "ё"
        @good_letters << bukva
      else
        @good_letters << bukva_alt
      end
